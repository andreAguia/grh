...
if ($acesso) {
    ...
    $page = new Page();
    $page->set_title("Declaração de Vinculo");
    $page->iniciaPagina();
    ...
    $salario = formataMoeda($pessoal->get_salarioBase($idServidorPesquisado));
    $faixa = $pessoal->get_faixaServidor($idServidorPesquisado);
    ...
    if ($idPerfil == 1) {
        if ($idSituacao == 1) {
            $texto .= " é {$texto2} desta Universidade, {$texto3} em {$dtAdmin}, através de Concurso Público, para o cargo de {$cargoEfetivo}, enquadrado na faixa/padrão salarial {$faixa} com os vencimentos de R$ {$salario}";
        } else {
