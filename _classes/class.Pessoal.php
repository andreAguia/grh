...
###########################################################
    ...
    /**
     * Método get_nomeEDescricaoCargo
     * fornece o nome e o cargo de um servidor
     * 
     * @param	string $idServidor idServidor do servidor
     */
    function get_nomeEDescricaoCargo($idServidor) {
        if (empty($idServidor)) {
            return null;
        } else {
            pLista(
                    $this->get_nome($idServidor),
                    $this->get_cargoComissaoDescricao($idServidor)
            );
        }
    }
     ...
     * @param	string $idServidor idServidor do servidor
     */
    function get_nomeECargoSimplesELotacao($idServidor) {
        if (empty($idServidor)) {
            return null;
        } else {
            pLista(
     ...
     * @param	string $idServidor idServidor do servidor
     */
    function get_nomeELotacaoESituacaoEAdmissao($idServidor) {
        if (empty($idServidor)) {
            return null;
        } else {
            ...
            if ($this->get_situacao($idServidor) == "Ativo") {
                span($this->get_situacao($idServidor), "verde");
                br();
                span($this->get_dtAdmissao($idServidor), " - ", "f11");
            } else {
                span($this->get_situacao($idServidor), "vermelho");
                br();
     ...
     * @param	string $idServidor  idServidor do servidor
     */
    function get_cargoComissaoAno($texto) {
        ...
        $select = "SELECT idComissao, tipo, dtNom, dtExo
                     FROM tbcomissao JOIN tbtiponomeacao ON (tbcomissao.tipo = tbtiponomeacao.idTipoNomeacao)
                    WHERE tbtiponomeacao.visibilidade <> 2
                      AND (({$ano} BETWEEN year(dtNom) AND year(dtExo))
                       OR ({$ano} >= year(dtNom) AND dtExo is null))
                    AND idServidor = {$idServidor}";
        ...
        foreach ($row as $rr) {
            ...
            if (!is_null($idComissao)) {
                ...
                # Coloca na variável retorno
                $retorno .= "{$descricao}<br/>({$tipoCargo})<br/>{$dataInicial} - {$dataFinal}";
                #$retorno .= "<span id='orgaoCedido'>[{$descricao}]</span>";
            }
