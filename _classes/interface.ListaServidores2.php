...
//        }
    ...
    public function showTabela() {
        ...
        if (($this->situacao == 1) AND ($this->situacaoSinal == "=")) {
            $label = ["ID/Matrícula", "Servidor", "Perfil", "Admissão", "Situação"];
            $width = [10, 40, 15, 15, 15];
            $function = [null, null, null, "date_to_php", $situacao];
        } else {
            $label = ["ID/Matrícula", "Servidor", "Perfil", "Admissão", "Saída", "Situação"];
            $width = [10, 40, 15, 15, 15, 15];
            $function = [null, null, null, "date_to_php", "date_to_php", $situacao];
        }

        $align = ["center", "left"];
        $classe = ["pessoal", "pessoal", "pessoal"];

        # Exibe o idServidor e idPessoa somente quando for para exibir
        if (is_null($this->matNomeId)) {
            $metodo = ["get_idFuncionalEMatricula", "get_nomeECargoELotacao", "get_perfil"];
        } else {
            if (is_null($this->idServidorIdPessoa)) {
                $metodo = ["get_idFuncionalEMatricula", "get_nomeECargoELotacao", "get_perfil"];
            } else {
